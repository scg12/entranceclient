...
@Component({ 
  selector: 'app-registrationsdp',
  templateUrl: './registrationsdp.component.html',
  styleUrls: ['./registrationsdp.component.css']
})
export class RegistrationsdpComponent implements OnInit {
 ...
 refreshItems(){
   ...
   getRegistration(){
    ...
    //.skipWhile(candidates => candidates.length < 3)
      .subscribe(
                  res=>{  
                          this.candidates = res.filter(x=>x.depositPlace ==this.depositP);
                          this.filteredItems = res.filter(x=>x.depositPlace ==this.depositP);
...
getDepositPlace() {
    ...
    .subscribe( 
      (response: string[]) =>{  var i =0;
                                ...
                                this.dps.push(response[i]['depositPlace']);                               
                            },
