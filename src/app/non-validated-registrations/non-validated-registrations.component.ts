...
@Component({
  selector: 'app-non-validated-registrations',
  templateUrl: './non-validated-registrations.component.html',
  styleUrls: ['./non-validated-registrations.component.css']
})
export class NonValidatedRegistrationsComponent implements OnInit {
    ...
    //.skipWhile(candidates => candidates.length < 3)
      .subscribe(
                  res=>{  //if(localStorage.getItem('priority')=="All")
                          ...
                          this.candidates = res.filter(x=>x.isValid==0);
                          this.filteredItems = this.candidates;
